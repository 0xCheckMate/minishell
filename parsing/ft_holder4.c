...
int	ft_create_holder_infile(t_line *tmp, t_holder *n, t_long *t)
{
	t_outfile	q;

	q.ee = 0;
	if (n && ft_is_there(tmp->str))
	{
		...
		if (q.qq >= 1)
		{
			while (q.a && q.a[q.ee])
			{
				n->file_in[(t->n)++] = ft_strdup(q.a[q.ee]);
				(1) && (q.ee++, n->nbr_file++, (t->index)++);
				n->infile_index[(t->sss)++] = t->index;
			}
			if (q.ee >= 1)
				return (ft_free_2d(q.a), ft_free_2d(n->file_in),
					ft_putstr_fd("ambiguous redirect\n", 2), 1);
		}
	}
	else if (n)
	{
		(1) && (n->file_in[(t->n)++] = ft_strdup(tmp->str), n->nbr_file++);
		(1) && ((t->index)++, n->infile_index[(t->sss)++] = t->index);
	}
	return (1);
}

int	ft_create_holder_append(t_line *tmp, t_holder *n, t_long *t)
{
	t_outfile	q;

	if (n && tmp && tmp->next && ft_is_there(tmp->next->str))
	{
		(1) && (q.ee = 0, tmp = tmp->next);
		(1) && (q.a = ft_does_it_matche(tmp->str), q.qq = ft_count_2d(q.a));
		if (q.qq >= 1)
		{
			while (n && q.a && q.a[q.ee])
			{
				n->append[(t->a)++] = ft_strdup(q.a[q.ee]);
				(1) && (q.ee++, (t->index)++, n->nbr_file++);
				n->append_index[(t->www)++] = t->index;
			}
			if (q.ee >= 1)
				return (ft_free_2d(q.a), ft_free_2d(n->append),
					ft_putstr_fd("ambiguous redirect\n", 2), 0);
		}
	}
	else if (n)
	{
		(1) && (n->append[(t->a)++] = ft_strdup(tmp->next->str), n->nbr_file++);
		(1) && ((t->index)++, n->append_index[(t->www)++] = t->index);
	}
	return (1);
}
